...
export default function AdminExams() {
  ...
  const [exportExamId, setExportExamId] = useState<string | undefined>(
    undefined,
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Schedule, monitor, and configure active examinations.
          </p>
        </div>
              ...
              <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          durationMinutes: e.target.value as any,
                        })
                      }
                      className="w-full p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    />
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 min-h-[100px]"
                    placeholder="Enter any specific instructions or anti-cheat warnings..."
                  ></textarea>
                </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 border border-indigo-100 bg-indigo-50/50 p-4 rounded-xl">
                    <div>
                      ...
                      <p className="text-xs text-indigo-700/70 mt-0.5">
                        Attach existing questions from the bank directly to this
                        exam
                      </p>
                    </div>
                    <Button
                      ...
                      className="mt-3 sm:mt-0 bg-white border-indigo-200 text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700"
                    >
                      <Library className="w-4 h-4 mr-2" /> Use Existing
                      Questions
                    </Button>
                  </div>

                  <h4 className="font-semibold text-slate-800 mb-3 text-sm uppercase tracking-wide mt-6">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            questionsPerCandidate: e.target.value as any,
                          })
                        }
          ...
          <Card className="w-full max-w-sm shadow-2xl border-red-100 ring-4 ring-red-100/50">
            <div className="p-6 text-center">
              ...
              <p className="text-slate-500 text-sm mb-6">
                ...
                <strong className="text-slate-700">{examToDelete.title}</strong>
                ? This will remove all associated statistics, scheduled timers,
                and candidate access permanently.
              </p>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {exams.map((exam) => (
          <Card
            key={exam.id}
            className={`border-t-4 transition-shadow hover:shadow-lg ${exam.status === "active" ? "border-green-500" : "border-slate-300"}`}
          >
            <CardHeader className="pb-2">
              <div className="flex justify-between items-start">
                ...
                <span
                  className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${exam.status === "active" ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-600"}`}
                >
                  {exam.status === "active" && (
            ...
            <CardContent>
              <div className="text-sm text-slate-600 space-y-2 mb-6">
                ...
                <p>
                  <strong>Department:</strong> {exam.department || "All"}
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-100">
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 shadow-sm flex-1 hover:bg-slate-50 transition-colors"
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 shadow-sm px-2 hover:bg-purple-50 hover:text-purple-600 transition-colors"
                  onClick={() =>
                    alert(
                      "Detailed stats for " +
                        exam.title +
                        " will be shown in the Analytics module.",
                    )
                  }
                >
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 shadow-sm px-2 hover:bg-slate-50 transition-colors"
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 shadow-sm px-2 hover:bg-blue-50 hover:text-blue-600 transition-colors"
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 shadow-sm px-2 hover:bg-red-50 hover:text-red-600 transition-colors"
        ...
        {exams.length === 0 && (
          <div className="col-span-full py-16 text-center text-slate-500 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50">
            <div className="flex flex-col items-center justify-center">
              ...
              <p className="text-sm text-slate-400 mt-1 max-w-sm mb-4">
                You haven't created any exams yet. Click the button below to get
                started.
              </p>
              <Button
                onClick={handleAdd}
