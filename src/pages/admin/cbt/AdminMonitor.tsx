...
import { Monitor, AlertTriangle, ShieldAlert, CheckCircle2, RefreshCw, Video } from 'lucide-react';
import { db } from '../../../lib/store';
import { ExamSession, Exam, User, Violation } from '../../../types';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <Card className="border-0 shadow-sm ring-1 ring-red-200 bg-red-50/50 cursor-pointer" onClick={() => setActiveTab('flagged')}>
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-2">
               <h3 className="text-red-800 font-semibold text-sm uppercase tracking-wider">Live Flagged Activities</h3>
               <AlertTriangle className="w-5 h-5 text-red-500" />
            </div>
            <p className="text-4xl font-extrabold text-red-700">{violations.length}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="shadow-sm border-0 ring-1 ring-slate-200">
        <div className="p-4 border-b border-slate-100 flex justify-between items-center">
          <h3 className="font-bold text-slate-800">
            {activeTab === 'flagged' ? 'Live Offense Feed' : 'Real-Time Session Feed'}
          </h3>
          {activeTab === 'flagged' && (
            <div className="flex gap-2">
                <Button size="sm" variant="outline" className="text-xs" onClick={() => alert("Admin anti-cheat settings can be found in General Settings.")}>Configure AI Engine</Button>
            </div>
          )}
        </div>
        <div className="p-0">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                {activeTab === 'flagged' ? (
                   <>
                     <th className="p-4 text-xs font-semibold tracking-wider text-slate-500 uppercase">Offense Type</th>
                     <th className="p-4 text-xs font-semibold tracking-wider text-slate-500 uppercase">Timestamp</th>
                   </>
                ) : (
                   <>
            ...
            <tbody className="divide-y divide-slate-100">
              {activeTab === 'flagged' ? (
                violations.length === 0 ? (
                  <tr><td colSpan={4} className="p-8 text-center text-slate-500">No suspicious activity detected.</td></tr>
                ) : (
                  violations.map(v => {
                    const user = users.find(u => u.id === v.candidateId);
                    const exam = exams.find(e => e.id === v.examId);
                    return (
                      <tr key={v.id} className="hover:bg-red-50/30 transition-colors animate-in fade-in slide-in-from-top-2">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-slate-100 border border-slate-200 overflow-hidden shrink-0 shadow-sm relative">
                              ...
                              <div className="absolute inset-0 bg-red-500/10 flex items-center justify-center -m-1">
                                <Video className="w-8 h-8 text-red-500/20" />
                              </div>
                            </div>
                            <div>
                              <p className="font-bold text-slate-800 leading-tight">{user?.name || 'Unknown Candidate'}</p>
                              <p className="text-xs text-blue-600 font-mono font-medium tracking-wide">{user?.serialNumber || v.candidateId}</p>
                            </div>
                          </div>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold uppercase tracking-wider bg-red-100 text-red-700 border border-red-200">
                             <AlertTriangle className="w-3.5 h-3.5" />
                             {v.description}
                          </span>
                        </td>
                        <td className="p-4">
                           <span className="text-xs font-mono text-slate-600">{new Date(v.timestamp).toLocaleTimeString()}</span>
                        </td>
                      </tr>
                    )
                  })
