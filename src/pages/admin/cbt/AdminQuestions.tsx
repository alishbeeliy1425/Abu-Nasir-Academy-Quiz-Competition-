      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2 mt-4 sm:mt-0">
          <Button className="bg-gradient-to-r from-indigo-900 via-purple-900 to-indigo-900 hover:from-indigo-800 hover:via-purple-800 hover:to-indigo-800 text-amber-400 border border-indigo-700/50 shadow-lg shadow-indigo-900/20" onClick={() => setIsAiModalOpen(true)}>
             <Sparkles className="w-4 h-4 mr-2 text-amber-400"/> Generate AI
          </Button>
      ...
      {isAiModalOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-2 sm:p-4 bg-slate-900/80 backdrop-blur-md overflow-y-auto">
          <div className="w-full max-w-xl relative group my-4 sm:my-8 shrink-0">
            ...
            <Card className="full relative bg-slate-900 border border-slate-800 shadow-2xl rounded-2xl overflow-hidden">
              ...
              <div className="p-8">
                <div className="flex items-center gap-4 mb-8">
                   ...
                   <div>
                     <h3 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-amber-200 to-amber-500">Abu Nasir AI Generator</h3>
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <select value={aiData.examId} onChange={e => setAiData({ ...aiData, examId: e.target.value })} className="w-full p-3 bg-slate-800/50 border border-slate-700 rounded-xl text-slate-200 focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 outline-none transition-all shadow-inner">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <select value={aiData.subject} onChange={e => setAiData({ ...aiData, subject: e.target.value })} className="w-full p-3 bg-slate-800/50 border border-slate-700 rounded-xl text-slate-200 focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 outline-none transition-all shadow-inner">
                        <option value="">Select subject...</option>
                        {Array.from(new Set([...subjects.map(s => s.name), ...questions.map(q => q.subject), "English", "Mathematics", "Biology"])).filter(Boolean).sort().map(s => <option key={s} value={s}>{s}</option>)}
                      </select>
                    </div>
                    <div className="space-y-2">
                      <label className="block text-xs font-semibold text-indigo-300 uppercase tracking-widest">Focus Topic</label>
                      <input type="text" value={aiData.topic} onChange={e => setAiData({ ...aiData, topic: e.target.value })} className="w-full p-3 bg-slate-800/50 border border-slate-700 rounded-xl text-slate-200 focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 outline-none transition-all shadow-inner placeholder:text-slate-600" placeholder="e.g., Quadratic Equations" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <select value={aiData.difficulty} onChange={e => setAiData({ ...aiData, difficulty: e.target.value })} className="w-full p-3 bg-slate-800/50 border border-slate-700 rounded-xl text-slate-200 focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 outline-none transition-all shadow-inner">
                ...
                <div className="flex justify-end gap-4 mt-10 pt-6 border-t border-slate-800">
                  ...
                  <Button className="bg-gradient-to-r from-amber-500 to-yellow-600 hover:from-amber-400 hover:to-yellow-500 text-slate-900 font-bold shadow-lg shadow-amber-500/25 border-0 px-6 py-6 h-auto" onClick={handleAiGenerate} disabled={isGenerating}>
                    {isGenerating ? (
                      <div className="flex flex-col items-center">
                        ...
                        <div className="w-full bg-slate-900/50 h-1.5 rounded-full mt-2 overflow-hidden">
                          <div className="h-full bg-slate-100 transition-all duration-300" style={{ width: `${generationProgress}%` }}></div>
                        </div>
                        <div className="text-[10px] mt-1 opacity-70">{generationProgress}% Complete</div>
                      </div>
                    ) : <><Sparkles className="w-5 h-5 mr-3" /> Initialize Generation</>}
                  </Button>
                </div>
              </div>
            </Card>
